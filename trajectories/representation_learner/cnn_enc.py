...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_outputs=8, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=nn.BatchNorm1d, input_channels=1):
        ...
        self.conv1 = nn.Conv1d(input_channels, self.inplanes, kernel_size=15, stride=2, padding=3,
                               bias=False)
...
#             print("Got to downsample place")
    ...
    def forward(self, x):
        op, intermediate_op = self._forward_impl(x)
        return op
